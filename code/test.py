...
class BNBuilder :
	...
	def unweighted_solutions( self ) :
		...
		num_nodes = len( self.data.fields )
		num_edges = sum( [ len( greedy_graph[ f ][ 'childs' ] ) for f in self.data.fields ] )
		density = float( num_edges ) / float( num_nodes )
		print "Number of attributes = %s" % num_nodes
		print "Number of edges = %s" % num_edges
		print "Density of overline G = %s" % density
		solutions = []
		return solutions
...
if __name__ == "__main__" :
	if len( sys.argv ) == 3 :
		...
		builder.buildNetwork( outfilepath = out_file % 'asdf' )
	else :
